...
export class ReportService {
  ...
  shadowRunStatusText(summary: ShadowRunTelegramSummary): string {
    const lines = [
      ...
      `- Policy runtime decision: ${summary.policyRuntimeDecision ?? 'TIDAK DIUJI'}`,
      `- Policy vs hint consistency: ${summary.policyVsHintConsistency ?? 'TIDAK DIUJI'}`,
      `- Policy guardrail enforced: ${summary.policyGuardrailEnforced ?? 'TIDAK DIUJI'}`,
